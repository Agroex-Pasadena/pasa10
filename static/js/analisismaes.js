...
var currentTab;
var values = {'sintomaBB': ['rojizo', 'moho verde azulado', 'moho negro', 'normal'],
              'sintomaAA': ['sintomaAAA', '<del>sintomaAAA</del>'],
              'sintomaCC': ['si empeora', 'no empeora']//,
            };
...
function submitForm() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/analisismaes', data: formData,
  ...
  for (i = 0; i < data.length; i++) {
    content += `
      <div id="card-`+i+`" class="enfermedad-card ui `+cardColors[index]+` card" style="display:none" onclick="clickCard(`+i+`)">
        <div class="image">
          <img src="/static/images/enfermedadesmaes/`+data[i].id+`_1.jpeg">
        </div>
        <div class="content">
...
function clickCard(id) {
  ...
  for (i = 1; i <= currentData[id].images; i++) {
    if (i == 1) {
      ...
      <div id="tab`+(i-1)+`" class="ui bottom attached tab active" data-tab="`+(i-1)+`">
        <img src="/static/images/enfermedadesmaes/`+currentData[id].id+`_`+i+`.jpeg" style="width:100%"/>
      </div>`;
    } else {
      imagesText += `
  ...
  $.ajax({ type: 'GET', url: '/api/reviewsmaes', data: {"enfermedadId": currentData[id].id},
    success: function(data) {
...
function submitReview() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/reviewsmaes', data: formData,
    success: function(data) {
