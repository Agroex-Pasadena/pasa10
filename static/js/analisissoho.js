...
var currentTab;
var values = {'sintomaBB': ['marron', 'violeta', 'centro claro borde oscuro', 'verde palido amarillento', 'no mancha'],
              'sintomaAA': ['sintomaAAA', '<del>sintomaAAA</del>'],
              'sintomaDD': ['si a amarillo', 'si a gris', 'no cambio color'],
              'sintomaCC': ['si halo', 'no halo'],
              'sintomaEE': ['si color vaina', 'no color vaina']//,
            };
...
function submitForm() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/analisissoho', data: formData,
  ...
  for (i = 0; i < data.length; i++) {
    content += `
      <div id="card-`+i+`" class="enfermedad-card ui `+cardColors[index]+` card" style="display:none" onclick="clickCard(`+i+`)">
        <div class="image">
          <img src="/static/images/enfermedadessoho/`+data[i].id+`_1.jpeg">
        </div>
        <div class="content">
...
function clickCard(id) {
  ...
  for (i = 1; i <= currentData[id].images; i++) {
    if (i == 1) {
      ...
      <div id="tab`+(i-1)+`" class="ui bottom attached tab active" data-tab="`+(i-1)+`">
        <img src="/static/images/enfermedadessoho/`+currentData[id].id+`_`+i+`.jpeg" style="width:100%"/>
      </div>`;
    } else {
      imagesText += `
  ...
  $.ajax({ type: 'GET', url: '/api/reviewssoho', data: {"enfermedadId": currentData[id].id},
    success: function(data) {
...
function submitReview() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/reviewssoho', data: formData,
    success: function(data) {
