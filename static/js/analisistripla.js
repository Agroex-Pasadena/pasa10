...
var currentTab;
var values = {'sintomaBB': ['verde', 'verde oscuro azulado', 'verde olivo', 'celeste con mancha', 'amarillo verdoso', 'otro'],
              'sintomaAA': ['sintomaAAA', '<del>sintomaAAA</del>'],
              'sintomaDD': ['si linea', 'no linea'],
              'sintomaCC': ['si globosa', 'no globosa'],
              'sintomaEE': ['en hoja', 'en raiz', 'en hoja y tallo', 'en otra parte']//,
            };
var changableData = ['sintomaAA', 'sintomaBB', 'sintomaCC', 'sintomaEE', 'sintomaDD'];
var cardColors = ["grey", "black", "grey", "black", "grey", "black"];
...
function submitForm() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/analisistripla', data: formData,
  ...
  for (i = 0; i < data.length; i++) {
    content += `
      <div id="card-`+i+`" class="enfermedad-card ui `+cardColors[index]+` card" style="display:none" onclick="clickCard(`+i+`)">
        <div class="image">
          <img src="/static/images/enfermedadestripla/`+data[i].id+`_1.jpeg">
        </div>
        <div class="content">
...
function clickCard(id) {
  ...
  for (i = 1; i <= currentData[id].images; i++) {
    if (i == 1) {
      ...
      <div id="tab`+(i-1)+`" class="ui bottom attached tab active" data-tab="`+(i-1)+`">
        <img src="/static/images/enfermedadestripla/`+currentData[id].id+`_`+i+`.jpeg" style="width:100%"/>
      </div>`;
    } else {
      imagesText += `
  ...
  $.ajax({ type: 'GET', url: '/api/reviewstripla', data: {"enfermedadId": currentData[id].id},
    success: function(data) {
...
function submitReview() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/reviewstripla', data: formData,
    success: function(data) {
