...
var currentTab;
var values = {'sintomaBB': ['estadio reproductivo', 'cualquier etapa'],
              'sintomaAA': ['sintomaAAA', '<del>sintomaAAA</del>'],
              'sintomaDD': ['decoloracion tallo', 'punto oscuro tallo','humeda y algodonosa tallo'],
              'sintomaCC': ['necrosis', 'decoloracion','ninguna lesion']//,
            };
...
function submitForm() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/analisissota', data: formData,
  ...
  for (i = 0; i < data.length; i++) {
    content += `
      <div id="card-`+i+`" class="enfermedad-card ui `+cardColors[index]+` card" style="display:none" onclick="clickCard(`+i+`)">
        <div class="image">
          <img src="/static/images/enfermedadessota/`+data[i].id+`_1.jpeg">
        </div>
        <div class="content">
...
function clickCard(id) {
  ...
  for (i = 1; i <= currentData[id].images; i++) {
    if (i == 1) {
      ...
      <div id="tab`+(i-1)+`" class="ui bottom attached tab active" data-tab="`+(i-1)+`">
        <img src="/static/images/enfermedadessota/`+currentData[id].id+`_`+i+`.jpeg" style="width:100%"/>
      </div>`;
    } else {
      imagesText += `
  ...
  $.ajax({ type: 'GET', url: '/api/reviewssota', data: {"enfermedadId": currentData[id].id},
    success: function(data) {
...
function submitReview() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/reviewssota', data: formData,
    success: function(data) {
